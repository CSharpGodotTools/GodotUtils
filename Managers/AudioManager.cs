...
public interface IGame<TSelf> where TSelf : IGame<TSelf>
{
    static abstract OptionsManager Options { get; }
}

public class AudioManager<TGame> : IDisposable
    where TGame : IGame<TGame>
{
    ...
    {
        _parent = parent;
        _options = TGame.Options.GetOptions();
    ...
    {
        _sfxPool = new Pool<AudioStreamPlayer2D>(_parent, () => new AudioStreamPlayer2D());
    }

    private void SetupMusicPlayer()
    {
        _musicPlayer = new AudioStreamPlayer();
        _parent.AddChild(_musicPlayer);
    }

...
public sealed class ComponentPool<TNode> where TNode : CanvasItem, IComponentPoolable<TNode>
{
    ...
    /// </summary>
    public TNode Acquire() => _core.Acquire(node => node.OnCreate(this), node => node.Components.SetActive(true));

    /// <summary>
    /// Releases the <paramref name="node"/> from the pool.
    /// </summary>
    public void Release(TNode node) => _core.Release(node, n => n.Components.SetActive(false));
